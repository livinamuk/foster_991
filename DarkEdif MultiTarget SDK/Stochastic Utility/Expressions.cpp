...
{
	return (Value > 0) ? (int)floorf(Value + 0.5f) : (int)ceilf(Value - 0.5f);
}
...
float Extension::Normalise(float Value, float Minimum, float Maximum, int LimitRange)
{
	...
	return (LimitRange != 0) ? Limit(Value, 0, 1) : Value;
}
...
float Extension::ModifyRange(float Value, float Minimum, float Maximum, float NewMinimum, float NewMaximum, int LimitRange)
{
	...
	return (LimitRange != 0) ? Limit(Value, NewMinimum, NewMaximum) : Value;
}
...
float Extension::Wave(int Waveform, float Value, float CycleStart, float CycleEnd, float Minimum, float Maximum)
{
	switch (Waveform)
	{
		...
		{
			// Sine
			return ModifyRange(sinf(ModifyRange(Value, CycleStart, CycleEnd, 0.f, 6.283185307f, 0)), -1.f, 1.f, Minimum, Maximum, 0);
		}

		case 1:
		{
			// Cosine
			return ModifyRange(cosf(ModifyRange(Value, CycleStart, CycleEnd, 0.f, 6.283185307f, 0)), -1.f, 1.f, Minimum, Maximum, 0);
		}
		...
		{
			// Triangle
			return Mirror(ModifyRange(Value, CycleStart, CycleStart + (CycleEnd - CycleStart) / 2.f, Minimum, Maximum, 0), Minimum, Maximum);
		}

		case 5:
		{
			// Square
			return (UberMod(Value, CycleStart, CycleEnd) < CycleStart + (CycleEnd - CycleStart) / 2.f) ? Minimum : Maximum;
		}
...
{
	return ModifyRange(EuclideanMod(Normalise(Dividend,Lower,Upper,0),1.f),0.f,1.f,Lower,Upper,0);
}
...
{
	if (From < To) {
		return From+fabsf(EuclideanMod(Value-To,(To-From)*2.f)-(To-From));
	} else {
		return To+fabsf(EuclideanMod(Value-From,(From-To)*2.f)-(From-To));
	}
}

float Extension::ExpressionCompare(float First, float Second, int ComparisonType, float ReturnIfTrue, float ReturnIfFalse)
{
	return Compare(First,Second,ComparisonType) ? ReturnIfTrue : ReturnIfFalse;
}

float Extension::Approach(float Value, float Amount, float Target)
{
	return (Value<Target) ? fmin(Value + Amount, Target) : fmax(Value - Amount, Target);
}
...
//String expressions
const TCHAR * Extension::Substr(const TCHAR * String, int Start, int Length)
{
	if (Start >= 0)
		String += Start;
	else
		String = (String + _tcslen(String)) + Start;

	if(Length >= 0)
	{
		std::tstring Return(String, Length);
		return Runtime.CopyString(Return.c_str());
	}

	int RealLength = _tcslen(String);

	std::tstring Return(String, RealLength);
	Return[RealLength + Length] = 0;

	return Runtime.CopyString(Return.c_str());
}

const TCHAR * Extension::StrExpressionCompare(float First, float Second, int ComparisonType, const TCHAR* ReturnIfTrue, const TCHAR* ReturnIfFalse)
{
