...
#if !defined(RUN_ONLY) && defined(_WIN32)
	delete Icon;
#endif
}
...
int ActionOrCondition(void* Function, int ID, Extension* ext, const ACEInfo* info, ACEParamReader& params, bool isCond)
{
...
ProjectFunc void expression(long extPtr, int ID, CNativeExpInstance expU)
{
	...
	int ParameterCount = Info->NumOfParams;
	long* Parameters = (long*)alloca(sizeof(long) * (ParameterCount + paramInc));
	memset(Parameters, 0, sizeof(int) * (ParameterCount + paramInc));
	long Result = 0;
