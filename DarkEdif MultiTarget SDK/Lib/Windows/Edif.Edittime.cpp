...
extern void IFO_GetExpressionParam(short code, short param, TCHAR * strBuf, short maxLen);
extern void IFO_GetExpressionTitle(short code, TCHAR * strBuf, short maxLen);
extern void IFO_GetExpressionString(short code, TCHAR * strPtr, short maxLen);
#endif
...
#else
		IFO_GetExpressionParam(code, param, strBuf, maxLen);
#endif
}
...
#else
		IFO_GetExpressionTitle(code, strBuf, maxLen);
#endif
	}
}
...
#else
		IFO_GetExpressionString(code, strPtr, maxLen);
#endif
}
...
void * DLLExport GetConditionInfos(mv *mV, short code)
{
	if (IS_COMPATIBLE(mV))
	{
		if ((std::uint16_t)code < ::SDK->ConditionInfos.size())
			return ::SDK->ConditionInfos[code]->MMFPtr();
		char missingID[256];
		sprintf_s(missingID, "A invalid condition ID %i was accessed. Please correct your JSON ConditionMenu.", code);
		MessageBoxA(NULL, missingID, "DarkEDIF - Crash prevented", MB_OK);
	}
	return NULL;
}
...
void * DLLExport GetActionInfos(mv * mV, short code)
{
	if (IS_COMPATIBLE(mV))
	{
		if ((std::uint16_t)code < ::SDK->ActionInfos.size())
			return ::SDK->ActionInfos[code]->MMFPtr();
		char missingID[256];
		sprintf_s(missingID, "A invalid action ID %i was accessed. Please correct your JSON ActionMenu.", code);
		MessageBoxA(NULL, missingID, "DarkEDIF - Crash prevented", MB_OK);
	}
	return NULL;
}
...
void * DLLExport GetExpressionInfos(mv * mV, short code)
{
	if (IS_COMPATIBLE(mV))
	{
		if ((std::uint16_t)code < ::SDK->ExpressionInfos.size())
			return ::SDK->ExpressionInfos[code]->MMFPtr();
		char missingID[256];
		sprintf_s(missingID, "A invalid expression ID %i was accessed. Please correct your JSON ExpressionMenu.", code);
		MessageBoxA(NULL, missingID, "DarkEDIF - Crash prevented", MB_OK);
	}
	return NULL;
}
